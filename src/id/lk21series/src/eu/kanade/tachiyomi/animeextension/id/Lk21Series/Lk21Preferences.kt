...
object Lk21Preferences {
    ...
    // Preference Keys
    const val PREF_GATEWAY_KEY = "gateway_url"
    ...
    // Default Values
    const val DEFAULT_GATEWAY_URL = "https://d21.team/"
    const val DEFAULT_BASE_URL_SERIES = "https://tv3.nontondrama.my"
    ...
    fun setupPreferences(
        ...
        EditTextPreference(screen.context).apply {
            key = PREF_GATEWAY_KEY
            title = "Gateway URL"
            setDefaultValue(DEFAULT_GATEWAY_URL)
            summary = preferences.getString(PREF_GATEWAY_KEY, DEFAULT_GATEWAY_URL)
            dialogTitle = "Enter Gateway URL"
            dialogMessage = "URL untuk fetch domain LK21 yang aktif (d21.team)"

            setOnPreferenceChangeListener { _, newValue ->
                val newUrl = (newValue as String).trimEnd('/') + "/"
                summary = newUrl
                // Clear domain cache when gateway changes
                preferences.edit()
                    .putString("cached_domain", "")
                    .apply()
                true
            }
    ...
    /**
     * Get gateway URL dari preferences
     */
    fun getGatewayUrl(preferences: SharedPreferences): String {
        return preferences.getString(PREF_GATEWAY_KEY, DEFAULT_GATEWAY_URL) ?: DEFAULT_GATEWAY_URL
    }
